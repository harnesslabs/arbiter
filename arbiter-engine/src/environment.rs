...
use crate::error::ArbiterEngineError;
  ...
  V: Clone + Send + Sync + 'static + DeserializeOwned,
{
  ...
  pub fn run(mut self) -> Result<JoinHandle<()>, ArbiterEngineError> {
  ...
  fn handle_transaction(&mut self, tx: Vec<u8>) -> Result<(), ArbiterEngineError> {
  ...
  fn get(&self, key: &K) -> Result<Option<V>, ArbiterEngineError> {
  ...
  fn sender(&self) -> mpsc::Sender<Vec<u8>> { self.tx_sender.clone() }
}
...
impl Middleware {
  ...
  pub async fn stream(&mut self) -> Result<impl Stream<Item = Vec<u8>>, ArbiterEngineError> {
...
where
  K: Serialize,
  V: Serialize, {
  serde_json::to_vec(&(key, value)).map_err(|e| {
    ArbiterEngineError::DatabaseError(format!("Failed to serialize transaction: {}", e))
  })
}
